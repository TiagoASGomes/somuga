...
public class ReviewControllerTest {
    ...
    @Test
    @DisplayName("Test create movie review and expect status 201 and like")
    void testCreateMovieReview() throws Exception {
        ReviewCreateDto reviewDto = new ReviewCreateDto(user.id(), movie.id(), 5, "My Review");
        ...
        ReviewMovieDto review = mapper.readValue(response, ReviewMovieDto.class);

        assertNotNull(review.id());
        assertEquals(reviewDto.userId(), review.user().id());
        assertEquals(reviewDto.mediaId(), review.media().id());
        assertEquals(user.userName(), review.user().userName());
        assertEquals(movie.title(), review.media().title());
        assertEquals(5, review.reviewScore());
        assertEquals("My Review", review.writtenReview());
    }
    ...
    void testGetAllReviewsFromUser() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            createReview(userId, createMovie().id(), org.somuga.enums.MediaType.MOVIE);
        }
